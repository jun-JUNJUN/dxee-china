#!/usr/bin/env python3
"""
Enhanced DeepSeek Web Research with MongoDB Caching and Multi-Query Strategy v3.06
...
New Features in v3.06 (Web Research System Optimization):
- Fixed iteration logic to properly trigger additional research rounds when relevance targets aren't met
- Fixed statistical summary generation to show proper data types and source attribution
- Fixed relevance scoring consistency between iteration and final analysis phases
- Enhanced content extraction reliability with retry logic and proper error handling
- Implemented robust token limit management and content optimization
- Added comprehensive fallback strategies for API errors and timeouts
- Improved time management with 10-minute session limits and early termination
- Enhanced analysis generation to prevent empty responses

Previous Features in v3.05:
...
Usage:
    python test_deepseek_advanced_web_research3_06.py
...
# Configure enhanced logging for v3.06
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    filename='deepseek_enhanced_research_v306.log',
    filemode='a'
)
...
# Constants for v3.06 optimization and bug fixes
...
@dataclass
class SearchMetrics:
    """Track search and analysis metrics with v3.05 enhancements"""
    total_queries: int = 0
    total_results: int = 0
    ...
    tokens_saved: int = 0
    statistical_data_found: int = 0  # v3.05: Track statistical data extraction
...
Generate 4-5 comprehensive search queries that approach this question from different angles. Focus on finding authoritative, data-rich sources."""
    ...
    async def analyze_content_with_gaps(self, original_question: str, extracted_contents: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Enhanced analysis with gap identification, relevance scoring, and token limit management (v3.05)"""
        start_time = time.time()
        
        logger.info("🧠 Starting comprehensive content analysis with token optimization")
        
        try:
            # v3.05: Optimize content for token limits
...
Note: Content has been optimized for analysis within token limits."""
            ...
            # v3.05: Handle token limit errors specifically
            if "max input limit" in error_str or "too long" in error_str.lower():
                logger.error(f"❌ Token limit exceeded in analysis (v3.05 optimization needed): {error_str}")
    ...
    async def generate_statistical_summary(self, original_question: str, extracted_contents: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Generate statistical summary using DeepSeek API reasoning based on actual data found in sources (v3.05)"""
        start_time = time.time()
        
        logger.info("📊 Starting statistical summary generation (v3.05)")
...
{chr(10).join(formatted_sources)}"""
    ...
    async def conduct_enhanced_research(self, original_question: str, target_relevance: int = 7, max_iterations: int = 3) -> Dict[str, Any]:
        ...
        while current_iteration < max_iterations and current_relevance < target_relevance:
            ...
            current_iteration += 1
            logger.info(f"🔄 Starting iteration {current_iteration}/{max_iterations} (current relevance: {current_relevance}/{target_relevance})")
            ...
            # Extract relevance score from analysis for iteration control
            iteration_relevance = 0
            if analysis and 'overall_relevance_score' in analysis:
                iteration_relevance = analysis['overall_relevance_score']
            
            # Update current relevance and iteration tracking
            current_relevance = iteration_relevance
            iteration_results['relevance_achieved'] = current_relevance
            iteration_results['statistical_summary'] = analysis.get('statistical_summary') if analysis else None
            
            # Track best iteration results
            if best_iteration_results is None or current_relevance > best_iteration_results.get('relevance_achieved', 0):
                best_iteration_results = iteration_results.copy()
                best_iteration_results['sources'] = all_sources.copy()
                best_iteration_results['analysis'] = analysis
            
            # Add sources from this iteration
            all_sources.extend(iteration_sources)
            
            logger.info(f"✅ Iteration {current_iteration} complete - Relevance: {current_relevance}/{target_relevance}")
            
            # Check if target relevance achieved
            if current_relevance >= target_relevance:
                logger.info(f"🎯 Target relevance {target_relevance} achieved in iteration {current_iteration}")
                break
                
        # End of iteration loop - use best results
        logger.info(f"🔄 Iteration process complete after {current_iteration} iterations")
        
        # Use best iteration results or fallback to latest
        final_iteration_results = best_iteration_results or iteration_results
        
        # Update results with final iteration data
        results.update({
            'iterations_performed': current_iteration,
            'final_relevance_achieved': current_relevance,
            'target_relevance': target_relevance,
            'best_iteration': final_iteration_results['iteration'] if final_iteration_results else current_iteration,
            'iteration_history': [iteration_results] if 'iteration_results' in locals() else []
        })
...
async def test_enhanced_research():
    """Test the iterative enhanced research process with MongoDB caching and statistical summaries (v3.05)"""
    print_separator()
    print("🚀 DEEPSEEK ENHANCED WEB RESEARCH v3.05 - WITH STATISTICAL SUMMARIES")
    print("Enhanced Multi-Query Research Process with Statistical Summary Generation (v3.05):")
    ...
    print_separator("=", 100)
    print("📊 ITERATION RESULTS WITH STATISTICAL SUMMARY GENERATION (v3.05)")
    ...
    # Fix: Look for statistical summary in the correct location
    statistical_summary = {}
    
    # First try iterations structure (v3.06 fix)
    iterations = results.get('iterations', [])
    if iterations and len(iterations) > 0:
        # Get statistical summary from the latest iteration
        latest_iteration = iterations[-1]
        statistical_summary = latest_iteration.get('statistical_summary', {})
    
    # Fallback to analysis structure 
    if not statistical_summary or not statistical_summary.get('summary_type'):
        analysis = results.get('analysis', {})
        if analysis:
            statistical_summary = analysis.get('statistical_summary', {})
    
    # Final fallback to final_metrics (legacy)
    if not statistical_summary or not statistical_summary.get('summary_type'):
        statistical_summary = final_metrics.get('statistical_summary', {})
    
    print_separator("=", 100) 
    print("🎉 FINAL RESULTS WITH STATISTICAL SUMMARY (v3.05)")
    ...
    # Statistical Summary Display (v3.06 - Fixed)
    print(f"\n📊 STATISTICAL SUMMARY GENERATION (v3.06):")
    
    # Enhanced summary type display with better fallbacks
    summary_type = statistical_summary.get('summary_type', '')
    if not summary_type:
        summary_type = 'not_available'
        print(f"   📈 Summary Type: Not Available (Generation failed or not attempted)")
    else:
        print(f"   📈 Summary Type: {summary_type.title()}")
    
    # Enhanced source count display
    source_count = statistical_summary.get('source_count', 0)
    if source_count == 0 and summary_type != 'no_data':
        print(f"   📄 Sources Used: 0 (Warning: No sources attributed - possible extraction failure)")
    else:
    ...
    if statistical_summary.get('error'):
        print(f"   ⚠️ Error: {statistical_summary.get('error')}")
    ...
    # Display statistical summary (v3.05)
    if statistical_summary and statistical_summary.get('summary_text'):
        print_separator("⭐", 100)
        print("📊 STATISTICAL SUMMARY (v3.05):")
    ...
    if target_achieved:
        print("🎉 SUCCESS: Target relevance score achieved with Statistical Summary generation (v3.06)!")
        print(f"✅ Research completed with {final_score}/10 relevance (≥{target_relevance} required)")
        summary_type = statistical_summary.get('summary_type', 'not_available')
        print(f"📊 Statistical summary generated: {summary_type.title() if summary_type != 'not_available' else 'Not Available'} analysis")
        ...
        summary_type = statistical_summary.get('summary_type', 'not_available')
        print(f"📊 Statistical summary: {summary_type.title() if summary_type != 'not_available' else 'Not Available'} analysis generated")
    ...
    filename = f'enhanced_research_v3_06_optimized_results_{timestamp}.json'
    ...
    # Cleanup
    await service.cleanup()
    print("🚀 Enhanced research with Statistical Summary generation (v3.05) completed!")
...
async def main():
    """Main test function"""
    print("🚀 DEEPSEEK ENHANCED WEB RESEARCH v3.06 - OPTIMIZATION & BUG FIXES")
    print("=" * 70)
