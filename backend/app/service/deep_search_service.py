#!/usr/bin/env python3
"""
Deep Search Service - Advanced Web Research with Multi-Step Process
Ported from test_deepseek_advanced_web_research.py for production use
"""
...
class DeepSearchService:
    """Advanced research service with multi-step process and progress callbacks"""
    ...
    async def step0_check_web_search_necessity(self, original_question: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self._notify_progress("step0_start", {"description": "Checking if web search is necessary"})
...
Please analyze this question and determine if web search is necessary. If web search is needed, respond with Query="search_terms". If not needed, provide a comprehensive answer directly."""
            ...
            self._notify_progress("step0_complete", result)
            return result
            ...
            self._notify_progress("step0_error", error_result)
            return error_result
    ...
    async def step2_web_search(self, search_query: str) -> List[Dict[str, Any]]:
        ...
        self._notify_progress("step2_start", {"description": "Performing web search", "query": search_query})
        
        results = await self.web_search.search(search_query, num_results=5)
        
        self._notify_progress("step2_complete", {"results": results, "count": len(results)})
        return results
    ...
    async def step3_extract_content(self, search_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        self._notify_progress("step3_start", {"description": "Extracting content from web pages", "total": len(search_results)})
        
        extracted_contents = []
        for i, result in enumerate(search_results):
            logger.info(f"Extracting content from result {i+1}/{len(search_results)}: {result['link']}")
            self._notify_progress("step3_progress", {"current": i+1, "total": len(search_results), "url": result['link']})
        ...
        self._notify_progress("step3_complete", {
    ...
    async def step4_analyze_relevance(self, original_question: str, extracted_contents: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        self._notify_progress("step4_start", {"description": "Analyzing content relevance with DeepSeek Reasoning"})
...
5. Overall assessment of how well the sources answer the original question"""
            ...
            self._notify_progress("step4_complete", result)
            return result
            ...
            self._notify_progress("step4_error", error_result)
            return error_result
    
    async def conduct_deep_search(self, original_question: str) -> Dict[str, Any]:
        """
        Conduct the complete deep search process with progress notifications
        ...
        """
        logger.info(f"Starting deep search for: {original_question}")
        self._notify_progress("search_start", {"question": original_question})
        ...
        try:
            ...
            if not necessity_check['web_search_needed']:
                ...
                self._notify_progress("search_complete", results)
                return results
            ...
            if not search_results:
                ...
                self._notify_progress("search_error", results)
                return results
